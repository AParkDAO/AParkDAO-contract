...
contract Migration is Ownable {
    ...
    function migrate() external onlyInitialized() {
        ...
        uint256 amount = alphaAPD.balanceOf(msg.sender);
        alphaAPD.transferFrom(msg.sender, address(this), amount);
        senderInfo[msg.sender] = senderInfo[msg.sender].add(amount);
        APD.transfer(msg.sender, amount);
    }
    ...
    function setStarted(uint256 _startTime) external onlyOwner() {
        startTime = _startTime;
    }
}