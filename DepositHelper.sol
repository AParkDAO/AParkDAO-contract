...
contract DepositHelper is Ownable {
    ...
    address public bouns;


    constructor (address _deposit, ISwapV2Router _router, address _LPAddress,
                    address _APD,address _USDT, ITreasury _treasury,address _bouns) {
        deposit = _deposit;
        APD = _APD;
        USDT = _USDT;
    ...
    function depositHelper(
        ...
        if (_tokenAddress == LPAddress) {
            ...
            APD.safeApprove(address(router), uint256(- 1));
            calAndSwap();
            (,, uint256 moreLPAmount) = router.addLiquidity(USDT, APD, USDT.myBalance(), APD.myBalance(), 0, 0, address(this), block.timestamp);
            LPAddress.safeApprove(address(deposit), moreLPAmount);
            if (USDT.myBalance() > 0) {
                USDT.safeTransfer(msg.sender, USDT.myBalance());
            }
            if (APD.myBalance() > 0) {
                APD.safeTransfer(msg.sender, APD.myBalance());
            }
            IDepositor(deposit).deposit(moreLPAmount, _maxPrice, msg.sender);
        }
        if(bouns != address(0))
            IBouns(bouns).inputData(msg.sender,_amount);
    }

    function depositValue(uint256 _amount) public view returns (uint256 value_) {
        ISwapV2Pair lpToken = ISwapV2Pair(factory.getPair(USDT, APD));
        ...
        if (swapAmt > 0) {
            ...
            (path[0], path[1]) = isReversed ? (APD, USDT) : (USDT, APD);
    ...
    function calAndSwap() internal {
        ...
        (uint256 debtReserve, uint256 relativeReserve) = USDT ==
        lpToken.token0() ? (token0Reserve, token1Reserve) : (token1Reserve, token0Reserve);
        (uint256 swapAmt, bool isReversed) = optimalDeposit(USDT.myBalance(), APD.myBalance(),
            debtReserve, relativeReserve);
        if (swapAmt > 0) {
            address[] memory path = new address[](2);
            (path[0], path[1]) = isReversed ? (APD, USDT) : (USDT, APD);
    ...
    function setBounsAddress(address _bouns) external onlyPolicy() {
        bouns = _bouns;
    }
