...
contract APEPARKERC20Token is ERC20Permit, VaultOwned {
    ...
    uint256 public fee = 120;

    uint256 public feeReward = 30;
    ...
    constructor() ERC20("APEPARK", "APD", 9) {

        router = IDEXRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
        USDT = 0xD8691a1A815a874E3094A8B5102C5cC9520cA8Ed;
        ...
        feeAddress = 0xCDf13985Af32f58a53804624c3DeD42EF89b4352;
        rewardAddress = 0xCDf13985Af32f58a53804624c3DeD42EF89b4352;
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
      uint256 _ariableAmount = amount;
      if(whiteListed[msg.sender] == false && recipient == pair){
        _ariableAmount = takeFee(msg.sender,amount);     
      }
      _transfer(msg.sender, recipient, _ariableAmount);
      return true;
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if(whiteListed[sender] == false && recipient == pair){
          _ariableAmount = takeFee(sender,amount);
        }
        _transfer(sender, recipient, _ariableAmount);
        _approve(sender, msg.sender, _allowances[sender][msg.sender].sub(amount, "ERC20: transfer amount exceeds allowance"));
        return true;
    }
    ...
    returns (uint256)
    {
        uint256 totalFee = fee.add(feeReward);
        uint256 feeAmount = amount.mul(totalFee).div(feeDenominator);
        _swap(sender,feeAmount,totalFee);
        return amount.sub(feeAmount);
    }

    function _swap(address sender,uint256 amount,uint256 totalFee) internal {
        ...
        IERC20(USDT).transfer(feeAddress, usdtBalance.mul(fee).div(totalFee));
        IERC20(USDT).transfer(rewardAddress, usdtBalance.mul(feeReward).div(totalFee));
    }
