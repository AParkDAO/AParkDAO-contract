...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SAPD }
    ...
    address public sAPD;
    uint public sAPDQueue; // Delays change to sAPD address
    ...
    constructor (
        ...
        isLiquidityToken[ _APDUSDT ] = true;
        liquidityTokens.push( _APDUSDT );

        bondCalculator[ _APDUSDT ] = _calculator;

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for APD
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint APD needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( APD ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn APD for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IAPDERC20( APD ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sAPD ).balanceOf( msg.sender ); // Can only borrow against sAPDheld
    ...
    /**
        @notice allow approved address to repay borrowed reserves with APD
        @param _amount uint
     */
    function repayDebtWithAPD( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IAPDERC20( APD ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, APD, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( APD ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( APD ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns APD valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match APD decimals
            value_ = _amount.mul( 10 ** IERC20( APD ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SAPD ) { // 9
            sAPDQueue = block.number.add( blocksNeededForQueue );
        } else return false;
